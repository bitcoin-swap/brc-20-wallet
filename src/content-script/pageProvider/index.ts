...
const log = (event, ...args) => {
  if (process.env.NODE_ENV !== 'production') {
    console.log(
      `%c [brc20wallet] (${new Date().toTimeString().slice(0, 8)}) ${event}`,
...
const channelName = script?.getAttribute('channel') || 'BRC20WALLET';
...
export class BRC20WalletProvider extends EventEmitter {
...
declare global {
  interface Window {
    brc20: BRC20WalletProvider;
  }
}

const provider = new BRC20WalletProvider();

if (!window.brc20) {
  window.brc20 = new Proxy(provider, {
    deleteProperty: () => true
  });
}

Object.defineProperty(window, 'brc20wallet', {
...
window.dispatchEvent(new Event('brc20wallet#initialized'));
